    ...
    internal class RocketEngine
    {
        ...
        public TypeOfFuel TypeFuel
        {
            ...
            set { if (Enum.IsDefined(typeof(TypeOfFuel), value)) typeFuel = value;
                else Console.Write("Такое топливо не найдено."); } 
        }
        ...
        public double SpeedTime(double time)
        {
            ...
            else if (time > time1 && time <= time2) return (((speed2 - speed1) / (time2 - time1)) * time);
            else if (time > time2 && time <= time3) return (((speed3 - speed2) / (time3 - time2)) * time);
            else return speed3;
        }
        public double Time()
        {
        ...
        public int NumberStep(double time)
        {
            ...
            else if (time > time1 && time <= time1+time2) return 2;
            else if (time > time2+time1 && time <= time1+time2+time3) return 3;
            else return -1;
        }
    } 
