...
do
{
    ...
    {
       if (geoSnake[0, 0] == geoSnake[i, 0] && geoSnake[0, 1] == geoSnake[i, 1])
        {
            health--;
            for (int j = 0; j < geoSnake.GetLength(1); j++)
                geoSnake[0, j] = snake.Length - j - 1;
            geo = ConsoleKey.RightArrow;
        }
...
while (true);
Console.WriteLine($"GAME OVER! Ваш счет: {score} очков.");